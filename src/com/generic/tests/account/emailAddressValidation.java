...
public class emailAddressValidation extends SelTestCase {
	...
	public void verifyEmailAddress(String caseId, String runTest, String desc, String proprties, String email,
			...
			String confirmEmailErrors, String passwordErrors) throws Exception {

		String url = PagesURLs.getHomePage() + PagesURLs.getEmailAddressPage();
		...
		this.email.set(getSubMailAccount((String) userDetails.get(Registration.keys.email)));
		...
		try {
			...
			if (proprties.contains("Verify current user")) {
				...
				if (proprties.contains("click update")) {
					...
					if (!confirmEmailErrors.equals("")) {
						String currentMsg = MyAccount_EmailAddress.getConfirmEmailErrorMsg();
						logs.debug(currentMsg);
						String confirmEmailErrorsMsg = MessageFormat.format(LoggingMsg.ACTUAL_EXPECTED_ERROR,
								currentMsg, confirmEmailErrors);
						sassert().assertTrue(currentMsg.contains(confirmEmailErrors), confirmEmailErrorsMsg);
					}
					if (!passwordErrors.equals("")) {
						String currentMsg = MyAccount_EmailAddress.getPasswordErrorMsg();
						logs.debug(currentMsg);
						String passwordErrorrMsg = MessageFormat.format(LoggingMsg.ACTUAL_EXPECTED_ERROR, currentMsg,
								passwordErrors);
						sassert().assertTrue(currentMsg.contains(passwordErrors), passwordErrorrMsg);
					}
				}
			}
