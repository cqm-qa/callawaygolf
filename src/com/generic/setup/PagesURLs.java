...
public class PagesURLs extends SelTestCase {
	...
	public static String getRegistrationPage() {
		try {
			return getCONFIG().getProperty(reg);
			} catch(Throwable t) {
				throw new NoSuchElementException(MessageFormat.format(LoggingMsg.PROPERTY_ERROR_MSG, reg));
			}
	}
	public static void setRegistrationPage(String newreg) {
		try {
			getCONFIG().setProperty(reg,newreg);
			} catch(Throwable t) {
				throw new NoSuchElementException(MessageFormat.format(LoggingMsg.PROPERTY_ERROR_MSG, reg));
			}
	}
	...
	public static String getLoginPage() {
		try {
			return getCONFIG().getProperty(login);
			} catch(Throwable t) {
				throw new NoSuchElementException(MessageFormat.format(LoggingMsg.PROPERTY_ERROR_MSG, login));
			}
	}
	public static void setLoginPage(String newlogin) {
		try {
			getCONFIG().setProperty(login,newlogin);
			} catch(Throwable t) {
				throw new NoSuchElementException(MessageFormat.format(LoggingMsg.PROPERTY_ERROR_MSG, login));
			}
	}
